...
exports.removeClub = async (req, res) => {
  try {
    ...
    let allClubString = JSON.stringify(allClub);
    console.log(allClubString);
    console.log(!allClubString.includes(clubName), "allClub no includes");
    
    let response;
    if (!allClubString.includes(clubName)) {
      ...
      let userClubString = JSON.stringify(userClubs);
      console.log(userClubs, "userClubs");

      if (!userClubString.includes(clubName)) {
        response = "You don't have such club"
      } else {
